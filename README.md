...
- Laser pointer (Control+0): a glowing, click-through dot with a configurable color and fading trail that follows the mouse pointer over any app, during zoom and drawing, and in recordings and screen shares. Esc or the hotkey turns it off. It hides the mouse pointer while on (Homebrew build) and turns itself off after a configurable idle time.
